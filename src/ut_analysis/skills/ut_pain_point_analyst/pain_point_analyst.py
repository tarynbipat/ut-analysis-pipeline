"""Pain point analyst skill implementation."""
...
THEME_KEYWORDS = {
    "Error and Failure Friction": ["error", "errors", "failed", "failure", "bug", "broken"],
    "Confusing Navigation": ["confusing", "confusion", "navigation", "find", "locate", "discover"],
    "Workflow Blockers": ["blocked", "blocker", "stuck", "prevent", "unable", "stop"],
    "Usability Friction": ["friction", "difficult", "hard", "slow", "tedious", "extra"],
    "Expectation Mismatch": ["unexpected", "surprised", "unclear", "misleading", "mismatch"],
}
THEME_IMPLICATIONS = {
    "Error and Failure Friction": "Fix the underlying failure points and make recovery paths clearer.",
    "Confusing Navigation": "Reduce navigation ambiguity so users can find the right path faster.",
    "Workflow Blockers": "Remove or redesign the blocking step so users can continue without help.",
    "Usability Friction": "Streamline the interaction to remove unnecessary effort and delay.",
    "Expectation Mismatch": "Clarify the interface so the experience matches user expectations.",
}
DEFAULT_IMPLICATION = "Fix or remove the friction behind {theme_title_lower} to reduce user pain."
FALLBACK_THEME_TITLE = "Pain Point Theme"


class PainPointAnalystSkill:
    """Analyzes pain points, frustrations, blockers, and usability barriers."""
    ...
    def analyze_lane(self, lane_data: dict[str, Any], run_id: str) -> dict[str, Any]:
        """Analyze findings assigned to the pain point lane."""
        try:
            lane_id = lane_data.get("lane_id", "ut_pain_point_analyst")
            ...
            result = {
                "run_id": run_id,
                "lane_id": lane_id,
                "lane_name": lane_data.get("lane_name", "Pain Points"),
            ...
            logger.error("Pain point analysis failed: %s", exc)
    ...
    def _generate_summary(self, findings: list[dict[str, Any]], theme_title: str) -> str:
        ...
        return (
            ...
            f"highlighting recurring friction, frustration, and blockers around {theme_title.lower()}."
        )
