"""User needs analyst skill implementation."""
...
THEME_KEYWORDS = {
    "Missing Core Capability": ["need", "needs", "missing", "wish", "want", "wanted"],
    "Efficiency Expectations": ["faster", "efficient", "quicker", "save", "shortcut", "automate"],
    "Guidance and Clarity": ["expect", "expected", "guidance", "help", "clear", "clarity"],
    "Customization Demand": ["custom", "customize", "flexible", "personalize", "preference"],
    "Visibility and Feedback": ["status", "feedback", "progress", "confirm", "visibility"],
}
THEME_IMPLICATIONS = {
    "Missing Core Capability": "Build or prioritize the missing capability users explicitly rely on.",
    "Efficiency Expectations": "Improve the workflow so users can achieve their goals with less effort.",
    "Guidance and Clarity": "Add guidance that meets user expectations at the moment of need.",
    "Customization Demand": "Improve flexibility so the product better matches different user preferences.",
    "Visibility and Feedback": "Provide clearer feedback so users know what the system is doing.",
}
DEFAULT_IMPLICATION = "Build or improve support for {theme_title_lower} to better meet user needs."
FALLBACK_THEME_TITLE = "User Need Theme"


class NeedsAnalystSkill:
    """Analyzes unmet user needs, desires, and expectations."""
    ...
    def analyze_lane(self, lane_data: dict[str, Any], run_id: str) -> dict[str, Any]:
        """Analyze findings assigned to the needs lane."""
        try:
            lane_id = lane_data.get("lane_id", "ut_needs_analyst")
            ...
            result = {
                "run_id": run_id,
                "lane_id": lane_id,
                "lane_name": lane_data.get("lane_name", "User Needs"),
                "agent_name": "needs-analyst",
            ...
            logger.error("Needs analysis failed: %s", exc)
    ...
    def _generate_summary(self, findings: list[dict[str, Any]], theme_title: str) -> str:
        ...
        return (
            ...
            f"highlighting unmet needs, desired capabilities, and expectations around {theme_title.lower()}."
        )
