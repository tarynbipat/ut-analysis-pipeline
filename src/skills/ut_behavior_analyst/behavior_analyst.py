"""Behavior analyst skill implementation."""
...
THEME_KEYWORDS = {
    "Repeated Workarounds": ["workaround", "manually", "manual", "copy", "paste", "repeat"],
    "Habitual Navigation": ["always", "usually", "habit", "routine", "navigate", "return"],
    "External Tool Dependence": ["excel", "spreadsheet", "notes", "email", "outside", "external"],
    "Verification Patterns": ["double", "check", "verify", "confirm", "review"],
    "Avoidance Behavior": ["avoid", "skip", "bypass", "ignore", "abandon"],
}
THEME_IMPLICATIONS = {
    "Repeated Workarounds": "Formalize the workaround with built-in support so users do not have to improvise.",
    "Habitual Navigation": "Support the repeated path with faster access and fewer redundant steps.",
    "External Tool Dependence": "Integrate or support the external behavior users depend on today.",
    "Verification Patterns": "Reinforce validation and status cues that support careful checking behavior.",
    "Avoidance Behavior": "Address the cause of avoidance so users no longer feel the need to bypass the flow.",
}
DEFAULT_IMPLICATION = (
    "Support or formalize {theme_title_lower} if it reflects a durable user behavior."
)
FALLBACK_THEME_TITLE = "Behavior Pattern Theme"


class BehaviorAnalystSkill:
    """Analyzes repeated behaviors, habits, and workarounds."""
    ...
    def analyze_lane(self, lane_data: dict[str, Any], run_id: str) -> dict[str, Any]:
        """Analyze findings assigned to the behavior lane."""
        try:
            lane_id = lane_data.get("lane_id", "ut_behavior_analyst")
            ...
            result = {
                "run_id": run_id,
                "lane_id": lane_id,
                "lane_name": lane_data.get("lane_name", "Behavioral Patterns"),
                "agent_name": "behavior-analyst",
            ...
            logger.error("Behavior analysis failed: %s", exc)
    ...
    def _generate_summary(self, findings: list[dict[str, Any]], theme_title: str) -> str:
        ...
        return (
            ...
            f"highlighting repeated behaviors, habits, and workarounds around {theme_title.lower()}."
        )
