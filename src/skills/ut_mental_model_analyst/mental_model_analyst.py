"""Mental model analyst skill implementation."""
...
THEME_KEYWORDS = {
    "Conceptual Mismatch": ["thought", "assumed", "expected", "meant", "supposed", "understood"],
    "Terminology Confusion": ["term", "label", "word", "language", "naming", "called"],
    "System Model Gaps": ["how", "works", "process", "behind", "logic", "model"],
    "Permission Misconceptions": ["access", "permission", "role", "admin", "share"],
    "Feature Misunderstanding": ["feature", "button", "setting", "tool", "option"],
}
THEME_IMPLICATIONS = {
    "Conceptual Mismatch": "Align the product model with user expectations or teach the difference clearly.",
    "Terminology Confusion": "Rename or explain terminology so it matches how users describe the domain.",
    "System Model Gaps": "Expose the system logic more clearly to reduce conceptual uncertainty.",
    "Permission Misconceptions": "Clarify access and role behavior so users can predict what will happen.",
    "Feature Misunderstanding": "Improve feature explanation so users build the right mental model faster.",
}
DEFAULT_IMPLICATION = (
    "Improve alignment or education around {theme_title_lower} so users understand the system."
)
FALLBACK_THEME_TITLE = "Mental Model Theme"


class MentalModelAnalystSkill:
    """Analyzes how users think about the system, including misconceptions and conceptual gaps."""
    ...
    def analyze_lane(self, lane_data: dict[str, Any], run_id: str) -> dict[str, Any]:
        """Analyze findings assigned to the mental model lane."""
        try:
            lane_id = lane_data.get("lane_id", "ut_mental_model_analyst")
            ...
            result = {
                "run_id": run_id,
                "lane_id": lane_id,
                "lane_name": lane_data.get("lane_name", "Mental Models"),
                "agent_name": "mental-model-analyst",
            ...
            logger.error("Mental model analysis failed: %s", exc)
    ...
    def _generate_summary(self, findings: list[dict[str, Any]], theme_title: str) -> str:
        ...
        return (
            ...
            f"highlighting mental models, conceptual gaps, and misconceptions around {theme_title.lower()}."
        )
